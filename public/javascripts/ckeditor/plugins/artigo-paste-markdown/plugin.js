...
(function() {
  ...
  ;var hashElement=function(wholeMatch,m1){var blockText=m1;blockText=blockText.replace(/\n\n/g,"\n");blockText=blockText.replace(/^\n/,"");blockText=blockText.replace(/\n+$/g,"");blockText="\n\n~K"+(g_html_blocks.push(blockText)-1)+"K\n\n";return blockText;};var _RunBlockGamut=function(text){text=_DoHeaders(text);var key=hashBlock("<hr />");text=text.replace(/^[ ]{0,2}([ ]?\*[ ]?){3,}[ \t]*$/gm,key);text=text.replace(/^[ ]{0,2}([ ]?\-[ ]?){3,}[ \t]*$/gm,key);text=text.replace(/^[ ]{0,2}([ ]?\_[ ]?){3,}[ \t]*$/gm,key);text=_DoLists(text);text=_DoGithubCodeBlocks(text);text=_DoCodeBlocks(text);text=_DoBlockQuotes(text);text=_HashHTMLBlocks(text);text=_FormParagraphs(text);return text;}
  ...
  CKEDITOR.plugins.add('artigo-paste-markdown',{init:function(editor){editor.addCommand('pasteFromMarkdown',new CKEDITOR.dialogCommand('pasteFromMarkdown'));editor.ui.addButton('PasteFromMarkdown',{label:'Paste Markdown',command:'pasteFromMarkdown',icon:this.path+'images/icon.png'});CKEDITOR.dialog.add('pasteFromMarkdown',function(editor){return{title:'Paste Markdown',minWidth:400,minHeight:200,contents:[{id:'general',label:'Content',elements:[{type:'html',html:'Enter the markdown content in the field below and click "Ok".'},{type:'textarea',id:'contents',label:'Markdown Text',validate:CKEDITOR.dialog.validate.notEmpty('The markdown text field cannot be empty.'),required:true,commit:function(data){converter=new Showdown.converter().makeHtml;data.contents=converter(this.getValue());}}]}],onOk:function(){var dialog=this,data={};this.commitContent(data);editor.insertHtml(data.contents);}};});}});
})();